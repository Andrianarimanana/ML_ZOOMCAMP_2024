...
with col1:
    ...
    slope = st.number_input("thal", min_value=10, max_value=100, value=1)
    fbs = st.selectbox("fbs", [0,1])
    exang = st.selectbox("exang", [0,1])
    restecg = st.number_input("restecg", min_value=1.0, max_value=50.0, value=1)
    
with col2:
    id = st.number_input("id", min_value=50, max_value=300, value=224)
    age = st.number_input("age", min_value=80, max_value=200, value=53)
    sex = st.st.selectbox("sex", [0,1])
    dataset = st.number_input("dataset", min_value=-20.0, max_value=50.0, value=0)
    cp = st.number_input("cp", min_value=0, max_value=100, value=0)
    
with col3:
    trestbps = st.number_input("trestbps", min_value=50, max_value=300, value=123.0)
